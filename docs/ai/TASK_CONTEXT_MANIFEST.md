...
| Scheduled capture activation / preflight | project brief, `AGENTS.md`, `CLAUDE.md` when applicable, `docs/ROADMAP.md`, `packages/api/src/prematch-capture-preflight.ts` (+ tests), `packages/api/src/prematch-snapshot-capture-cli.ts`, `docs/operations/PREMATCH_SNAPSHOT_CAPTURE_ACTIVATION.md`, `.github/workflows/prematch-snapshot-capture.yml`, `.env.example` | `live-results-sync.ts`, `prematch-snapshot-capture.ts` | production formula/Elo/xG constants, standings, migrations, web UI | targeted preflight tests, API typecheck/build, `git diff --check` |
| Match context read model | project brief, `AGENTS.md`, `CLAUDE.md` when applicable, `docs/ROADMAP.md`, `packages/api/src/match-context.ts` (+ tests), `docs/data-quality/MATCH_CONTEXT_READ_MODEL.md`, `live-group-standings.ts`, `schemas.ts` match-context types | `docs/architecture/REAL_STANDINGS_AND_MATCH_CONTEXT_PLAN.md`, `tournament-form.ts` | prediction formula/Elo/xG constants, migrations, web UI, unrelated API handlers | targeted match-context tests, API typecheck, `git diff --check` |
| Match context UI & provenance | project brief, `AGENTS.md`, `CLAUDE.md` when applicable, `docs/ROADMAP.md`, `packages/api/src/daily-matches.ts`, `packages/api/src/group-detail.ts`, `packages/api/src/schemas.ts` (DailyMatchEntry), `apps/web/src/components/MatchContextDisplay.tsx` (+ test), `apps/web/src/components/DailyMatchCard.tsx`, `apps/web/src/components/GroupDetailMatchCard.tsx`, `apps/web/src/components/MatchSimulationResults.tsx`, `apps/web/src/components/PredictionHistoryDashboard.tsx`, `docs/dashboard/MATCH_CONTEXT_UI_AND_PROVENANCE.md` | `packages/api/src/match-context.ts`, `apps/web/src/lib/api-client.ts` | prediction formula/Elo/xG constants, snapshot identity, persistence migrations, provider selection, projection behavior | API typecheck, API tests, web typecheck, web tests, `git diff --check` |
