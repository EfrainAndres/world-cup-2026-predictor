...
**Phase 12.18A2 (complete):** An implementation phase that activated the automated pre-match snapshot capture against the `football_data_org` external provider and made it safely schedulable. Added `runPreMatchCaptureActivationPreflight()` (`packages/api/src/prematch-capture-preflight.ts`) — a pure injectable preflight service that validates configuration (PERSISTENCE_PROVIDER=postgres, DATABASE_URL, RESULTS_PROVIDER=football_data_org, FOOTBALL_DATA_API_TOKEN), database connectivity, live provider connectivity, fixture kickoff data, and capture window evaluability, returning a typed `ScheduledCaptureActivationStatus` and `PreflightChecks` breakdown without printing secrets — and `assessProviderReadiness()` for counting fixture quality metrics from a sync result. Extended the capture CLI (`prematch-snapshot-capture-cli.ts`) with `PREMATCH_CAPTURE_MODE=preflight|dry_run|capture` (default `capture`). Added `.github/workflows/prematch-snapshot-capture.yml` (30-minute schedule + workflow_dispatch, `WC2026_DATABASE_URL`/`FOOTBALL_DATA_API_TOKEN` secrets, `permissions: contents: read`, concurrency group). Activation verdict: `ready_to_activate` — all infrastructure checks pass; `activated_and_verified` requires a live upcoming fixture in the provider's fixture list. See `docs/operations/PREMATCH_SNAPSHOT_CAPTURE_ACTIVATION.md`.

**Phase 12.18A1 (complete):** An implementation phase added automated pre-match snapshot capture (`captureWorldCup2026PreMatchSnapshots` / `runScheduledPreMatchSnapshotCapture` in `packages/api/src/prematch-snapshot-capture.ts`) that discovers eligible upcoming World Cup 2026 fixtures, generates one deterministic prediction per fixture via the existing production `predictMatchFromLiveElo` path, and persists an immutable pre-match snapshot before kickoff. A named capture policy with explicit constants defines a 24h→15m capture window, typed eligibility states with deterministic millisecond boundary behaviour (ineligible exactly at kickoff), a bounded `maxFixturesPerRun`, the `balanced` preset, and tournament adjustments off by default. A stable fixture-derived capture identity (`cutoffAt = kickoff`) reuses the existing snapshot idempotency key and content hash so repeated scheduler runs never duplicate, a final pre-persistence guard enforces `capturedAt < kickoff`, and failures are reported with sanitized issue codes that never leak provider/DB errors. A dry-run mode performs no writes; a process-local mutex and a PostgreSQL `pg_try_advisory_lock` advisory lock prevent overlapping runs (scheduled mode requires PostgreSQL and never falls back to memory). A CLI (`capture:prematch-snapshots`, cron/Actions/Cron-ready) requires `DATABASE_URL` in postgres mode, never prints secrets, and exits non-zero on unsafe configuration or partial failure. No production formula, Elo/xG constant, standings logic, provider, migration, or UI prediction behaviour changed and no snapshot is ever updated or deleted; captured snapshots are compatible with the Model-vs-Reality evaluator and the Prediction Usefulness Audit. See `docs/model-results/AUTOMATED_PREMATCH_SNAPSHOT_CAPTURE.md`.
