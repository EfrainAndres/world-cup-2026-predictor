...
## Historical Monte Carlo Replay Scope

The historical Monte Carlo replay foundation maps Elo snapshot ratings into expected goals, generates Poisson score matrices, runs seeded repeated tournament simulations, and scores simulated champion probabilities against actual outcomes.

Current replay simulations use explicit simplified tournament fixtures. Full historical group and bracket reconstruction is still future work, so outputs are foundation evidence and not public model accuracy.

## Historical Bracket Reconstruction Scope

The historical bracket reconstruction foundation rebuilds the 2010, 2014, 2018, and 2022 World Cup structures from complete curated fixture results.
...
## Historical Replay Accuracy Audit Scope

The historical replay accuracy audit foundation summarizes the current replay system across 2010, 2014, 2018, and 2022.

It checks metric availability for Brier Score, Log Loss, Top-1, Top-3, and Top-5; validates dataset, bracket, Elo snapshot, Monte Carlo replay, and replay-validation readiness; detects foundation-only warnings; lists known gaps; and returns an API readiness recommendation.

The audit is a release-readiness tool, not a real predictive accuracy claim.

## Live Elo Pipeline Scope
...
- Calibrated Elo ratings to expected goals.
...
- Historical Monte Carlo replay from calibrated pre-tournament match probabilities.
